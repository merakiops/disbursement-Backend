...
class DashboardRepository:
    ...
    def get_dashboard_summary(client_ids: List[int], from_date, to_date, data_source: Optional[str] = "all", db: Session = None):
        ...
        # Standard aggregation query matching hover stats logic
        summary_sql = f"""
            SELECT 
                COUNT(DISTINCT td.country_id) FILTER (WHERE td.country_id IS NOT NULL) as countries,
                COUNT(DISTINCT td.port_id) FILTER (WHERE td.port_id IS NOT NULL) as ports,
                ...
                -- Awaiting / Yet to process FDA (where PDA exists or port call active, but no FDA recorded)
                COUNT(td.disbursement_seq) FILTER (WHERE fda.fda_id IS NULL AND pda.pda_id IS NOT NULL) as yet_to_process,
        ...
        result_row = db.execute(text(summary_sql), params).mappings().first()
        prod_summary = dict(result_row) if result_row else {}

        # Fetch Ankkumam data and merge if applicable
        prod_cid_to_excel, excel_to_prod_cid = DashboardRepository._get_dynamic_client_mapping(db)
        ankkumam_clients = []
        if client_ids:
            for cid in client_ids:
        ...
        if ankkumam_clients:
            ...
            if kamba_vessels:
                params["kamba_vessels"] = kamba_vessels
            ...
            if kamba_countries:
                params["kamba_countries"] = kamba_countries
            ...
            if kamba_ports:
                params["kamba_ports"] = kamba_ports
