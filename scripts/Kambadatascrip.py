#!/usr/bin/env python3
"""
Updated_merged_tracker_kamba_8key (2).xlsx -> Postgres. One self-contained script.

Same table, same schema, same trigger as the 2024 loader. Only the Excel header
mapping changed, so the merged tracker/Kamba sheet lands in the same columns.
...
What it does, in order:
  ...
  3. Check whether the table, counter, function and trigger exist. Create what's missing.
     Any column missing from an older table (pic, ops_pic, source) is added.
  4. Read the Excel file.
  5. Insert the rows. Rows already present are skipped, so re-running is safe.

Every row gets an auto-generated unique id: MDA<year>_<serial>, e.g. MDA2025_01.
...
Usage:
    ...
    python setup_and_load_merged.py

    # or without an env var:
    python setup_and_load_merged.py --db-url 'postgresql://...'
...
DEFAULT_TABLE = "data"
DEFAULT_EXCEL = "merged_lme_finel.xlsx"
DEFAULT_SHEET = "Merged"

# (Excel header, db column, pg type) in sheet order, left to right.
#
# Mapping notes, this is the only part that changed from the 2024 loader:
#   - the merged sheet's first column is called 'mda_id' and holds the source
#     system's own id. It goes to column_1, exactly the slot the 2024 sheet's
#     unnamed first column used. The table's mda_id stays the trigger-generated
#     primary key, so the two never collide.
#   - 'pic', 'ops_pic' and 'source' do not exist in the 2024 sheet. They are
#     added to the table as text columns.
#   - every other header is the same field under a snake_case name.
COLUMNS = [
    ("mda_id",                   "column_1",                 "text"),
    ("client",                   "client",                   "text"),
    ("final_status",             "final_status",             "text"),
    ("pic",                      "pic",                      "text"),
    ("ops_pic",                  "ops_pic",                  "text"),
    ("agency_nomination_date",   "agency_nomination_date",   "date"),
    ("invoice_no",               "invoice_no",               "text"),
    ("pda",                      "pda",                      "text"),
    ("pda_received_date",        "pda_received_date",        "date"),
    ("pda_status",               "pda_status",               "text"),
    ("pda_processing_date",      "pda_processing_date",      "date"),
    ("pda_payment_status",       "pda_payment_status",       "text"),
    ("fda",                      "fda",                      "text"),
    ("fda_received_date",        "fda_received_date",        "date"),
    ("fda_status",               "fda_status",               "text"),
    ("vessel",                   "vessel",                   "text"),
    ...
    ("eta",                      "eta",                      "date"),
    ("etd",                      "etd",                      "date"),
    ("port_agent",               "port_agent",               "text"),
    ("estimated_amount_softmar", "estimated_amount_softmar", "text"),
    ("pda_amount",               "pda_amount",               "text"),
    ("remittance_details",       "remittance_details",       "text"),
    ("fda_amount_usd",           "fda_amount_usd",           "numeric(18,2)"),
    ("agents_roe",               "agents_roe",               "numeric(18,6)"),
    ("actual_roe_oanda",         "actual_roe_oanda",         "numeric(18,6)"),
    ("roe_loss_usd",             "roe_loss_usd",             "numeric(18,2)"),
    ("detailed_entry_softmar",   "detailed_entry_softmar",   "text"),
    ("ws_chart_ac_in_softmar",   "ws_chart_ac_in_softmar",   "text"),
    ("owners_items_rejected",    "owners_items_rejected",    "text"),
    ("towage_agency_agreement",  "towage_agency_agreement",  "text"),
    ("fda_processing_date",      "fda_processing_date",      "date"),
    ("days_outstanding",         "days_outstanding",         "numeric(10,2)"),
    ("column_36",                "column_36",                "text"),
    ("remarks",                  "remarks",                  "text"),
    ("savings_at_pda_usd",       "savings_at_pda_usd",       "numeric(18,2)"),
    ("savings_at_fda_usd",       "savings_at_fda_usd",       "numeric(18,2)"),
    ("total_savings_usd",        "total_savings_usd",        "numeric(18,2)"),
    ("reason",                   "reason",                   "text"),
    ("source",                   "source",                   "text"),
]
...
def parse_number(v):
    ...
    s = str(v).strip()
    # Remove all characters except digits, periods, and minus signs
    s = re.sub(r'[^\d.-]', '', s)
    
    if not s or s == '-':
        return None
    # The merged sheet carries some numbers as text ('02', '1.0'). Accept those.
    try:
        f = float(s)
    except ValueError:
...
def read_sheet(path, sheet=None):
    ...
    def norm(h):
        """Headers are compared case- and space-insensitively."""
...
def build_ddl(schema, table, pad, per_year):
    ...
    # A table created by the 2024 loader has no pic / ops_pic / source. Adding
    # every data column here is a no-op for the ones already there.
    add_cols = "\n".join(
        f'ALTER TABLE {q}."{table}" ADD COLUMN IF NOT EXISTS {c} {t};' for _, c, t in COLUMNS)
...
-- Upgrade path: columns the 2024 sheet did not have.
{add_cols}
...
CREATE INDEX IF NOT EXISTS idx_{table}_source ON {q}."{table}" (source);
""".strip() + "\n"
...
def main():
    ...
    ap.add_argument("--pad", type=int, default=2,
                    help="minimum serial width (2 -> MDA2025_01, 4 -> MDA2025_0001)")
