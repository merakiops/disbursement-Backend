#!/usr/bin/env python3
"""
Avanah.xlsx -> Postgres. One self-contained script.

Same schema, same table, same trigger as the other loaders. The table is NOT
changed in any way: this sheet simply carries fewer columns, and every table
column the sheet does not have is left NULL.
...
Usage:
    ...
    python setup_and_load_avanah.py

    # or without an env var:
    python setup_and_load_avanah.py --db-url 'postgresql://...'
...
# ---------------------------------------------------------------- the sheet
#
# (Excel header, db column) in sheet order, left to right. 25 columns.
# Everything the Avanah sheet does not carry - voyage, cargo, invoice_no,
# agency_nomination_date, pda_payment_status, fda_status, estimated_amount_softmar,
# agents_roe, actual_roe_oanda, roe_loss_usd, detailed_entry_softmar,
# ws_chart_ac_in_softmar, owners_items_rejected, towage_agency_agreement,
# savings_at_pda_usd, ops_pic, column_1, column_36 - is simply left out of the
# INSERT, so those columns come out NULL.
#
# Three mappings worth knowing about:
#   'AVANAH'               -> client   (the header is the client name itself)
#   'FDA Savings'          -> savings_at_fda_usd
#   'MERAKI SAVINGS (AED)' -> total_savings_usd
#   'Meraki Saving Reason' -> reason
SHEET_MAP = [
    ("FINAL STATUS",                          "final_status"),
    ("AVANAH",                                "client"),
    ("PIC",                                   "pic"),
    ("PDA",                                   "pda"),
    ("PDA received from OPS / AGENTS",        "pda_received_date"),
    ("PDA Status",                            "pda_status"),
    ("PDA Processing Date",                   "pda_processing_date"),
    ("FDA",                                   "fda"),
    ("FDA received from OPS / AGENTS",        "fda_received_date"),
    ("Vessel",                                "vessel"),
    ("PORT",                                  "port"),
    ("COUNTRY",                               "country"),
    ("PURPOSE",                               "purpose"),
    ("ETA",                                   "eta"),
    ("ETD",                                   "etd"),
    ("PORT AGENT",                            "port_agent"),
    ("PDA Amount",                            "pda_amount"),
    ("Remittance Details\n( Currency Details )", "remittance_details"),
    ("FDA Amount",                            "fda_amount_usd"),
    ("FDA Processing Date(Eiger Completion)", "fda_processing_date"),
...
def parse_number(v):
    ...
    if not s:
        return None
    # Some sheets carry numbers as text ('02', '1.0', '1,234.5'). Accept those.
    try:
        f = float(s.replace(",", ""))
    except ValueError:
...
def main():
    ...
    ap.add_argument("--source", default=None,
                    help="value for the source column, e.g. --source Avanah (default: NULL)")
    ...
    if args.source is not None:
        data = [rec + [args.source] for rec in data]
        load_cols = load_cols + ["source"]
    print(f"  {len(data)} rows x {len(DATA_COLS)} mapped columns"
          f"  ({len(TABLE_COLUMNS) - len(DATA_COLS)} table columns stay NULL)\n")
    ...
    casts = sql.SQL(", ").join(
        ...
        sql.SQL(", source") if args.source is not None else sql.SQL(""))
