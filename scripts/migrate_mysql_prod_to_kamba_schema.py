...
logger = logging.getLogger("migration_prod")

def get_mysql_prod_engine():
    db_url = os.getenv("MYSQL_DB_URL_PROD")
    if not db_url:
        logger.error("MYSQL_DB_URL_PROD not found in .env file.")
        return None
    try:
        ...
        logger.error(f"Error creating MySQL Prod Engine: {e}")
        return None
...
def run_migration():
    logger.info("Starting MySQL PROD to PostgreSQL 'kamba_data_prod' schema migration...")
    
    mysql_engine = get_mysql_prod_engine()
    if not mysql_engine:
        logger.error("Could not connect to MySQL PROD RDS engine. Aborting.")
        sys.exit(1)
        
    target_schema = "kamba_data_prod"
    
    with postgres_engine.connect() as pg_conn:
        logger.info(f"Creating schema '{target_schema}' in PostgreSQL if not exists...")
    ...
    logger.info(f"Found {len(mysql_metadata.tables)} tables in MySQL PROD database.")
    ...
    for table_name, mysql_table in mysql_metadata.tables.items():
        ...
        # 1. Build PostgreSQL table definition in schema 'kamba_data_prod'
        pg_metadata = MetaData(schema=target_schema)
        pg_columns = []
        ...
        # Drop table if exists and recreate in PostgreSQL 'kamba_data_prod'
        with postgres_engine.connect() as pg_conn:
            pg_conn.execute(text(f"DROP TABLE IF EXISTS {target_schema}.\"{table_name}\" CASCADE;"))
            pg_conn.commit()
            pg_table.create(pg_conn)
            pg_conn.commit()
            
        # 2. Fetch rows from MySQL PROD table and insert into PostgreSQL 'kamba_data_prod'
        with mysql_engine.connect() as my_conn:
            ...
            logger.info(f"Migrating {mysql_count} rows for table '{table_name}' into {target_schema}...")
            ...
            logger.info(f"Successfully migrated {inserted_count}/{mysql_count} rows into {target_schema}.\"{table_name}\".")
    ...
    print("="*70)
    if mismatches == 0:
        logger.info(f"MIGRATION COMPLETED SUCCESSFULLY WITH 100% DATA INTEGRITY INTO '{target_schema}'!")
    else:
