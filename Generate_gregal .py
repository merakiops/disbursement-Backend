...
PORTS = [
    {
        ...
        "deductions": [
            ("Document Allowance",             "06 May 2026  22:54", "07 May 2026  00:54", "2",  ""),
            ("Others",                          "05 May 2026  13:24", "06 May 2026  00:42", "5.65",  "UAV Attack"),
            ("Others",                          "07 May 2026  1:18", "07 May 2026  02:48", "0.75",  "UAV Attack"),
        ],
        "summary": ("110.90 hrs", "\u2212 8.4 hrs", "102.5 hrs"),
    },
    {
...
LAYTIME_SUMMARY = [
    ...
    ("bg-ice",   "Less: Total Deductions",               "neg",  "17.20 hrs"),
    ("bg-white", "Allowed Laytime (SHINC)",              "val",  "96.00 hrs"),
    ("bg-ice",   "Additional / Extra Laytime",           "val",  "7.70 hrs"),
    ("bg-wake",  "Time on Demurrage",                    "blue", "154.6 hrs"),
]

FINANCIALS = [
    ("Gross Demurrage", "USD 1,288,333.3333", False),
    ("Less: Undisputed Demurrage",                          "USD 0.00",        False),
    ("Less: Address Commission @ 2.5%",                     "USD 32,208.3333", True),
]

NET_LABEL = "NET DEMURRAGE DUE"
NET_VALUE = "USD 1,256,125.00"
...
def draw_page(canvas, doc):
    ...
    canvas.setFillColor(C_GOLD)
    canvas.drawString(px, H - 1.60*cm, DOC_TITLE)
...
def deduction_table(deds, W_body):
    ...
    if has_remarks:
        cw = [W_body*0.28, W_body*0.19, W_body*0.19, W_body*0.10, W_body*0.24]
        hdr = [P('Deduction Type',th), P('From',thr), P('To',thr),
               P('Hrs',thr), P('Remarks',th)]
        rows = [[P(d[0],tc), P(d[1],tcr), P(d[2],tcr),
                 P(d[3], S(f'dr{i}','Helvetica-Bold',7.5,C_RED,TA_RIGHT)),
                 P(d[4], tci)] for i,d in enumerate(deds)]
    else:
        cw = [W_body*0.38, W_body*0.22, W_body*0.24, W_body*0.16]
        hdr = [P('Deduction Type',th), P('From',thr), P('To',thr), P('Hrs Deducted',thr)]
        rows = [[P(d[0],tc), P(d[1],tcr), P(d[2],tcr),
                 P(d[3], S(f'dr{i}','Helvetica-Bold',7.5,C_RED,TA_RIGHT))]
